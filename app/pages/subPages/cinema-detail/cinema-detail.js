...
Page({
  ...
  selectMovie(e) {
    ...
    movie.movieDays.forEach(item => {
      days.push({
        title: util.formatDate(new Date(item.day.replace(/-/g, '/'))),
        day: item.day
      })
    })
    this.setData({
  ...
  createEndTime(arr, dur,version) {
    ...
    if (Array.isArray(arr)) {
      timeList = arr.map(item => {
        ...
        console.log(item.startTime.replace(/-/g, '/'));
        var time = new Date(item.startTime.replace(/-/g, '/'))
        console.log(time);
        ...
        if(time.startTime != undefined){
          time.startTime = time.startTime.replace(/-/g, '/')
        }
        return temp
      })
    }
