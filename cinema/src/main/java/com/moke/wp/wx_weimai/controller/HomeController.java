...
import com.moke.wp.wx_weimai.entity.Vo.*;
...
public class HomeController {
    ...
    //获取首页电影待映列表
    @GetMapping("/getRankMovieWait")
    public Result getRankMovieWait(){
        List<Comment> adminComments = commentMapper.selectRank();
        List<Movie> result = new ArrayList<>(10);
        for (Comment adminComment : adminComments) {
            Movie movie = movieMapper.selectById(adminComment.getMovieId());
            result.add(movie);
        }
